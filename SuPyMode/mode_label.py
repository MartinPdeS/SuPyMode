...
for azimuthal, radial in [(0, 1), (1, 1), (2, 1), (0, 2), (3, 1), (1, 2), (4, 1), (2, 2), (0, 3), (5, 1), (3, 2), (1, 3), (6, 1), ]:
    ...
    if azimuthal == 0:
        parities = [[BoundaryValue.SYMMETRIC, BoundaryValue.SYMMETRIC]]
        sublabels = ['']

    elif azimuthal % 2 == 1:
        parities = [
            [BoundaryValue.ANTI_SYMMETRIC, BoundaryValue.SYMMETRIC],
            [BoundaryValue.SYMMETRIC, BoundaryValue.ANTI_SYMMETRIC],
        ]
        sublabels = ['_a', '_b']

    elif azimuthal % 2 == 0:
        parities = [
            [BoundaryValue.SYMMETRIC, BoundaryValue.SYMMETRIC],
            [BoundaryValue.ANTI_SYMMETRIC, BoundaryValue.ANTI_SYMMETRIC],
        ]
        sublabels = ['_a', '_b']
...
class ModeLabel:
    ...
    def get_filtered_mode_list(self) -> list[tuple]:
        ...
        """
        return [m['mode'] for m in mode_dict if self.x_parity in [m['x'], BoundaryValue.ZERO] and self.y_parity in [m['y'], BoundaryValue.ZERO]]
    ...
    def _get_parity(self, boundary_1: str, boundary_2: str) -> str:
        ...
        """
        if BoundaryValue.SYMMETRIC in (boundary_1, boundary_2):
            return BoundaryValue.SYMMETRIC
        elif BoundaryValue.ANTI_SYMMETRIC in (boundary_1, boundary_2):
            return BoundaryValue.ANTI_SYMMETRIC
        else:
            return BoundaryValue.ZERO
