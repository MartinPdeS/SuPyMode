...
get_rho_gradient_py(const pybind11::array_t<double> &mesh_py, const pybind11::array_t<double> &x_vector_py, const pybind11::array_t<double> &y_vector_py) {
    ...
    Eigen::VectorXd x_vector = NumpyInterface::convert_py_to_eigen(x_vector_py, x_size);
    Eigen::VectorXd y_vector = NumpyInterface::convert_py_to_eigen(y_vector_py, y_size);
    Eigen::MatrixXd mesh = NumpyInterface::convert_py_to_eigen(mesh_py, y_size, x_size);

    Eigen::MatrixXd rho_gradient = get_rho_gradient(mesh, y_vector, x_vector);

    return NumpyInterface::eigen_to_ndarray<double>(rho_gradient, {y_size, x_size});
}
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> Geometry::compute_gradient_2p() {
    int y_size = this->mesh.rows();
    int x_size = this->mesh.cols();
    ...
    for (int y_index = 0; y_index < y_size; ++y_index) {
        for (int x_index = 0; x_index < x_size; ++x_index) {
            ...
            double dxVal = 0.0;
            if (x_index> 0 && x_index + 1 < x_size) {
                dxVal = (this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index - 1)) / (2 * dx);
            } else if (x_index == 0 && x_index + 2 < x_size) {  // Use forward difference at the left boundary
                dxVal = (-3 * this->mesh(y_index, x_index) + 4 * this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index + 2)) / (2 * dx);
            } else if (x_index == x_size - 1 && x_index - 2 >= 0) {  // Use backward difference at the right boundary
                dxVal = (3 * this->mesh(y_index, x_index) - 4 * this->mesh(y_index, x_index - 1) + this->mesh(y_index, x_index - 2)) / (2 * dx);
            } else if (x_index == 0 && x_index + 1 < x_size) {  // Forward difference when there is only one neighboring point
                dxVal = (this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index)) / dx;
            } else if (x_index == x_size - 1 && x_index - 1 >= 0) {  // Backward difference when there is only one neighboring point
                dxVal = (this->mesh(y_index, x_index) - this->mesh(y_index, x_index - 1)) / dx;
            }

            // Compute dyVal
            double dyVal = 0.0;
            if (y_index> 0 && y_index + 1 < y_size) {
                dyVal = (this->mesh(y_index + 1, x_index) - this->mesh(y_index - 1, x_index)) / (2 * dy);
            } else if (y_index == 0 && y_index + 2 < y_size) {  // Use forward difference at the top boundary
                dyVal = (-3 * this->mesh(y_index, x_index) + 4 * this->mesh(y_index + 1, x_index) - this->mesh(y_index + 2, x_index)) / (2 * dy);
            } else if (y_index == y_size - 1 && y_index - 2 >= 0) {  // Use backward difference at the bottom boundary
                dyVal = (3 * this->mesh(y_index, x_index) - 4 * this->mesh(y_index - 1, x_index) + this->mesh(y_index - 2, x_index)) / (2 * dy);
            } else if (y_index == 0 && y_index + 1 < y_size) {  // Forward difference when there is only one neighboring point
                dyVal = (this->mesh(y_index + 1, x_index) - this->mesh(y_index, x_index)) / dy;
            } else if (y_index == y_size - 1 && y_index - 1 >= 0) {  // Backward difference when there is only one neighboring point
                dyVal = (this->mesh(y_index, x_index) - this->mesh(y_index - 1, x_index)) / dy;
            }
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> Geometry::compute_gradient_5p() {
    size_t y_size = this->mesh.rows();
    size_t x_size = this->mesh.cols();
    ...
    for (size_t y_index = 0; y_index < y_size; ++y_index) {
        for (size_t x_index = 0; x_index < x_size; ++x_index) {
            ...
            // Calculate dxVal
            if (x_index >= 2 && x_index + 2 < x_size) {
                dxVal = (this->mesh(y_index, x_index - 2) - 8 * this->mesh(y_index, x_index - 1) + 8 * this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index + 2)) / (12 * dx);
            } else if (x_index > 0 && x_index + 1 < x_size) {
                dxVal = (this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index - 1)) / (2 * dx);
            } else if (x_index == 0 && x_index + 1 < x_size) {
                dxVal = (this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index)) / dx;
            ...
            // Calculate dyVal
            if (y_index >= 2 && y_index + 2 < y_size) {
                dyVal = (this->mesh(y_index - 2, x_index) - 8 * this->mesh(y_index - 1, x_index) + 8 * this->mesh(y_index + 1, x_index) - this->mesh(y_index + 2, x_index)) / (12 * dy);
            } else if (y_index > 0 && y_index + 1 < y_size) {
                dyVal = (this->mesh(y_index + 1, x_index) - this->mesh(y_index - 1, x_index)) / (2 * dy);
            } else if (y_index == 0 && y_index + 1 < y_size) {
                dyVal = (this->mesh(y_index + 1, x_index) - this->mesh(y_index, x_index)) / dy;
...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> Geometry::compute_gradient_7p() {
    size_t y_size = this->mesh.rows();
    size_t x_size = this->mesh.cols();
    ...
    for (size_t y_index = 0; y_index < y_size; ++y_index) {
        for (size_t x_index = 0; x_index < x_size; ++x_index) {
            double dxVal = 0.0;
            if (x_index>= 3 && x_index + 3 < x_size) {
                dxVal = (-this->mesh(y_index, x_index - 3) + 9 * this->mesh(y_index, x_index - 2) - 45 * this->mesh(y_index, x_index - 1) + 45 * this->mesh(y_index, x_index + 1) - 9 * this->mesh(y_index, x_index + 2) + this->mesh(y_index, x_index + 3)) / (60 * dx);
            } else if (x_index> 0 && x_index + 1 < x_size) {
                dxVal = (this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index - 1)) / (2 * dx);  // Central difference
            } else if (x_index == 0 && x_index + 2 < x_size) {  // Forward difference at the left boundary
                dxVal = (-3 * this->mesh(y_index, x_index) + 4 * this->mesh(y_index, x_index + 1) - this->mesh(y_index, x_index + 2)) / (2 * dx);
            } else if (x_index == x_size - 1 && x_index - 2 >= 0) {  // Backward difference at the right boundary
                dxVal = (3 * this->mesh(y_index, x_index) - 4 * this->mesh(y_index, x_index - 1) + this->mesh(y_index, x_index - 2)) / (2 * dx);
            ...
            double dyVal = 0.0;
            if (y_index>= 3 && y_index + 3 < y_size) {
                dyVal = (-this->mesh(y_index - 3, x_index) + 9 * this->mesh(y_index - 2, x_index) - 45 * this->mesh(y_index - 1, x_index) + 45 * this->mesh(y_index + 1, x_index) - 9 * this->mesh(y_index + 2, x_index) + this->mesh(y_index + 3, x_index)) / (60 * dy);
            } else if (y_index > 0 && y_index + 1 < y_size) {
                dyVal = (this->mesh(y_index + 1, x_index) - this->mesh(y_index - 1, x_index)) / (2 * dy);  // Central difference
            } else if (y_index == 0 && y_index + 2 < y_size) {  // Forward difference at the top boundary
                dyVal = (-3 * this->mesh(y_index, x_index) + 4 * this->mesh(y_index + 1, x_index) - this->mesh(y_index + 2, x_index)) / (2 * dy);
            } else if (y_index == y_size - 1 && y_index - 2 >= 0) {  // Backward difference at the bottom boundary
                dyVal = (3 * this->mesh(y_index, x_index) - 4 * this->mesh(y_index - 1, x_index) + this->mesh(y_index - 2, x_index)) / (2 * dy);
...
Eigen::MatrixXd Geometry::get_rho_gradient_time_rho() {
    auto gradients = this->compute_gradient_5p();
    Eigen::MatrixXd gradient_x = gradients.first;
    Eigen::MatrixXd gradient_y = gradients.second;

    Eigen::MatrixXd rho_gradient(this->y.size(), this->x.size());

    double angle, cos_angle, sin_angle;

    for (long x_index = 0; x_index < this->x.size(); ++x_index) {
        for (long y_index = 0; y_index < this->y.size(); ++y_index) {
            double _x = this->x(x_index);
            double _y = this->y(y_index);
            double rho = std::sqrt(pow(_x, 2) + pow(_y, 2));

            angle = std::atan2(_y, _x);
            cos_angle = std::cos(angle);
            sin_angle = std::sin(angle);
...
Eigen::MatrixXd Geometry::get_rho_gradient(){
