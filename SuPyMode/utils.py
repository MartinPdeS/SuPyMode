...
def interpret_slice_number_and_itr(
    ...
    slice_from_itr = (
        itr_to_slice(itr_baseline, itr=itr_list)
        if itr_list.size > 0
        else numpy.array([])
    )
...
def parse_filename(save_function: Callable) -> Callable:
    @wraps(save_function)
    def wrapper(superset, filename: str, directory: str = "auto", **kwargs):
...
def load_superset(filename: str, directory: str = "default"):
    """
...
def slice_to_itr(itr_list: numpy.ndarray, slice_number: int | list) -> float | list:
    ...
    raise TypeError("slice_number must be a scalar or a list of scalars.")
