...
class SuPySolver(object):
    ...
    def generate_dataframe(self, wavelength, Nstep, Nsol, ITRf, tolerance, error):
        ...
        arrays0 = [['std', 'std', 'std', 'std', 'std', 'std', 'std', 'std',],
                  ['ITR', 'index','order', 'beta', 'Field', 'x_symmetry', 'y_symmetry', 'name']]

        arrays1 = [['Coupling']*self.Nsol,
                    list(range(self.Nsol))]

        arrays2 = [['Adiabatic']*self.Nsol,
                    list(range(self.Nsol))]

        arrays3 = [['Overlap']*self.Nsol,
                    np.arange(self.Nsol).astype(int)]
    ...
    def stack_results(self, sol_list: list):
        ...
        for solution in sol_list:
            ...
            self.Fields.loc[(self.iter,solution)][('std','index')] = self.betas[solution] / self.Axes.real['k']
    ...
    def concatenate_figure(self, mode: int, iter: int):

        image = self.Fields.loc[(iter,mode)][('std','Field')].reshape(self.info['shape']).astype('float')

        x_axis, y_axis = self.Axes.real['xvector'], self.Axes.real['yvector']
