...
fiber_list = [
    fiber_loader.load_fiber(
        "DCF1300S_20",
        clad_refractive_index=clad_refractive_index,
        position=clad_structure.cores[0],
    ),
    fiber_loader.load_fiber(
        "DCF1300S_26",
        clad_refractive_index=clad_refractive_index,
        position=clad_structure.cores[1],
    ),
    fiber_loader.load_fiber(
        "DCF1300S_33",
        clad_refractive_index=clad_refractive_index,
        position=clad_structure.cores[2],
    ),
]
...
workflow = Workflow(
    ...
    boundaries=[
        Boundaries()
    ],  # Set of symmetries to be evaluated, each symmetry add a round of simulation
...
profile = AlphaProfile(symmetric=False, add_end_of_taper_section=False)

# %%
# Adding a first taper segment with large initial heating length (i.e. slow reduction)
profile.add_taper_segment(
    alpha=0, initial_heating_length=0.1e-3, stretching_length=0.1e-3
)

profile.initialize()

propagation = workflow.superset.propagate(
    add_coupling=True, profile=profile, initial_amplitude=[1, 0, 0]
)
